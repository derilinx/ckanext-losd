    ...
    def conversion_for_old_jstat_version():
        ...
        try:
            ...
            for data_field_nm in dimensions['id']:
                ...
                scheme.append('' + vocabulary_namespace_prefix + ':' + _cleanString(data_field_nm) + ' a qb:ComponentProperty, qb:DimensionProperty ;\n\trdfs:label "' + data_field_nm + '" ;\n\trdfs:range xsd:string .\n\n')

            scheme.append('' + vocabulary_namespace_prefix + ':value a qb:ComponentProperty, qb:MeasureProperty ;\n\trdfs:label "value" ;\n\trdfs:range xsd:float .\n\n')

            # Scheme: DSD

            scheme.append('' + data_namespace_prefix + ':' + _cleanString(dataset_label) + '_dsd a qb:DataStructureDefinition ;\n\tqb:component\n\t\t[ a qb:ComponentSpecification ;\n\t\t  qb:codeList ' + data_namespace_prefix + ':conceptscheme/measureType ; \n\t\t  qb:dimension qb:measureType ;\n\t\t  qb:order 1 \n\t] ;\n\tqb:component [ qb:measure ' + vocabulary_namespace_prefix + ':value ] ;\n\t')

            for index, data_field_nm in enumerate(dimensions['id']):

                scheme.append('qb:component\n\t\t[ a qb:ComponentSpecification ;\n\t\t  qb:codeList ' + data_namespace_prefix + ':conceptscheme/' + _cleanString(data_field_nm) + ' ;\n\t\t  qb:dimension ' + vocabulary_namespace_prefix + ':' + _cleanString(data_field_nm) + ' ;\n\t\t  qb:order ' + str(index + 2) + ' \n\t\t] ')
            ...
            # Code list: Concept scheme

            for data_field_nm in dimensions['id']:
                code_list.append('' + data_namespace_prefix + ':conceptscheme/' +
                                 _cleanString(data_field_nm) + ' a skos:ConceptScheme ;\n\t')


                skos_members = []
                for concept in dimensions[data_field_nm]['category']['label'].values():
                    skos_members.append('skos:member ' + urlize(':concept', data_field_nm, concept)+ ' ')
            ...
            for data_field_nm in dimensions['id']:
                for concept in dimensions[data_field_nm]['category']['label'].values():
                    code_list.append('' + urlize(':concept', data_field_nm, concept) +
                                     ' a skos:Concept ;\n\trdfs:label "' + concept + '" .\n\n')
            ...
            for t in xrange(total_size):
                observations.append('[ a qb:Observation ;\n\tqb:dataSet ' + data_namespace_prefix + ':' +
                ...
                for index, data_field_nm in enumerate(dimensions['id']):
                    ...
                    observations.append('' + data_namespace_prefix + ':' + 'concept/' + _cleanString(data_field_nm) \
                                    + '/' + all_term[index][tracker[index]] + ' ;\n\t')
            ...
            with open(turtle_file.name, 'w') as out_file:
                out_file.write(''.join(scheme).encode("utf-8"))
                out_file.write(''.join(code_list).encode("utf-8"))
                out_file.write(''.join(observations).encode("utf-8"))
    ...
    def conversion_for_new_jstat_version():
        ...
        try:
            ...
            for data_field_nm in field_nms:
                scheme.append('' + vocabulary_namespace_prefix + ':' + _cleanString(data_field_nm) +
                              ' a qb:ComponentProperty, qb:DimensionProperty ;\n\trdfs:label "' +
                              data_field_nm + '" ;\n\trdfs:range xsd:string .\n\n')

            scheme.append('' + vocabulary_namespace_prefix +
                          ':value a qb:ComponentProperty, qb:MeasureProperty ;\n\trdfs:label "value" ;\n\trdfs:range xsd:float .\n\n')

            # Scheme: DSD

            scheme.append('' + data_namespace_prefix + ':' + _cleanString(dataset_label) +
                          '_dsd a qb:DataStructureDefinition ;\n\tqb:component\n\t\t[ a qb:ComponentSpecification ;\n\t\t  qb:codeList '
                          + data_namespace_prefix + ':conceptscheme/measureType ; \n\t\t  qb:dimension qb:measureType ;\n\t\t  qb:order 1 \n\t] ;\n\tqb:component [ qb:measure '
                          + vocabulary_namespace_prefix + ':value ] ;\n\t')

            for index, data_field_nm in enumerate(field_nms):
                if data_field_nm != 'Units':
                    scheme.append('qb:component\n\t\t[ a qb:ComponentSpecification ;\n\t\t  qb:codeList ' +
                                  data_namespace_prefix + ':conceptscheme/' + _cleanString(data_field_nm)
            ...
            for data_field_nm in field_nms:
                if data_field_nm != 'Units':
                    ...
                    for concept in dimensions[data_field_nm]['category']['label'].values():
                        skos_members.append(
                            'skos:member ' + urlize(':concept', data_field_nm, concept) + ' ')
            ...
            for t in xrange(total_size):
                ...
                for data_field_nm in field_nms:
                    ...
                    if data_field_nm != 'Units':
                        ...
                        observations.append('' + vocabulary_namespace_prefix + ':'
                                            + _cleanString(data_field_nm) + ' ')
                        observations.append('' + data_namespace_prefix + ':' + 'concept/' +
                                            _cleanString(data_field_nm) + '/' +
                ...
                observations.append('qb:measureType ' + vocabulary_namespace_prefix + ':value ;\n\t' +
                                    vocabulary_namespace_prefix + ':value "' + str(dataset_values[t]) +
                                    '"^^xsd:float\n] . \n\n')
